        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${gdriveEnabled ? 'bg-brand-600' : 'bg-gray-200 dark:bg-gray-700'} ${user?.role !== 'admin' && user?.role !== 'compliance_officer' ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={user?.role !== 'admin' && user?.role !== 'compliance_officer'}
              className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${notionEnabled ? 'bg-brand-600' : 'bg-gray-200 dark:bg-gray-700'} ${user?.role !== 'admin' && user?.role !== 'compliance_officer' ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
